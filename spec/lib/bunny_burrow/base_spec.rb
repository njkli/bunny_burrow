...
describe BunnyBurrow::Base do
  ...
  let(:tls_key)             { 'tls-key' }
  let(:tls_ca_certificates) { [ 'tls-ca-cert' ] }
  ...
  describe 'custom initialization' do
    ...
    subject do
      described_class.new do |dc|
        ...
        dc.tls_key = tls_key
        dc.tls_ca_certificates = tls_ca_certificates
      end
    end
    ...
    it 'allows tls_ca_certificates to be set' do
      expect(subject.tls_ca_certificates).to eq(tls_ca_certificates)
    end
  ...
  describe 'instance' do
    ...
    describe '#connection' do
      before(:each) do
        ...
        allow(subject).to receive(:tls_ca_certificates).and_return(tls_ca_certificates)
      ...
      it 'sets tls_ca_certificates when present' do
        expect(Bunny).to receive(:new).with(anything, hash_including(tls_ca_certificates: tls_ca_certificates))
        subject.send :connection
      end

      it 'does not set tls_ca_certificates when not present' do
        allow(subject).to receive(:tls_ca_certificates).and_return(nil)
        expect(Bunny).to receive(:new).with(anything, hash_excluding(tls_ca_certificates: tls_ca_certificates))
        subject.send :connection
      end
